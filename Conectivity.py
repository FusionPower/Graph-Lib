...
def dfs(node,adj,vis,component=1):
    vis[node]=component
    for n in adj[node]:
        if not vis[n]:
            dfs(n,adj,vis,component)
...
def is_tree(adj,edj):
    return is_connected(adj) and len(edj)==len(adj)-1
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.QUIT:
            ...
            if event.key==pygame.K_1:
                print("Is my graph connected?")
                print(is_connected(adj))
            if event.key==pygame.K_2:
                print("Number of conected components")
                print(connected_components(adj))
            if event.key==pygame.K_3:
                print("Is this a tree?")
                print(is_tree(adj, edj))
